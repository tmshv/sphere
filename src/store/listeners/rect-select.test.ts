...
describe("rect-select listener", () => {
    ...
    test("rectSelectDrag for MVT source uses client-only path (selectionSet, not selectionRect)", async () => {
        const mvtSource = {
            type: "MVT",
            format: "pbf",
            sourceLayers: [{ id: "layer1", name: "Layer 1" }],
        }
        const { store } = makeStore({ source: { selectedId: "src-1", items: { "src-1": mvtSource } } })

        const { assignFeatureIds } = await import("@/lib/maplibre")
        const mockAssign = vi.mocked(assignFeatureIds)
        mockAssign.mockReturnValue({
            ids: [1, 2],
            json: '[{"type":"Feature","id":1},{"type":"Feature","id":2}]',
            fc: { type: "FeatureCollection", features: [] },
        })

        invokeMock.mockImplementation((cmd: string) => {
            if (cmd === "selection_cache_features") return Promise.resolve()
            if (cmd === "selection_set") return Promise.resolve({ added: [1, 2], removed: [] })
        ...
        expect(invokeMock.mock.calls.some((c: unknown[]) => c[0] === "selection_cache_features")).toBe(true)
        expect(invokeMock.mock.calls.some((c: unknown[]) => c[0] === "selection_set")).toBe(true)
        expect(invokeMock.mock.calls.some((c: unknown[]) => c[0] === "selection_rect")).toBe(false)
        expect(invokeMock.mock.calls.some((c: unknown[]) => c[0] === "selection_rect_features")).toBe(false)
    })

    test("rectSelectCommit for MVT source uses client-only path with apply + sync", async () => {
        const mvtSource = {
            type: "MVT",
            format: "pbf",
            sourceLayers: [{ id: "layer1", name: "Layer 1" }],
        }
        const { store, dispatched } = makeStore({ source: { selectedId: "src-1", items: { "src-1": mvtSource } } })

        const { assignFeatureIds } = await import("@/lib/maplibre")
        const mockAssign = vi.mocked(assignFeatureIds)
        mockAssign.mockReturnValue({
            ids: [1],
            json: '[{"type":"Feature","id":1}]',
            fc: { type: "FeatureCollection", features: [] },
        })

        invokeMock.mockImplementation((cmd: string) => {
            if (cmd === "selection_cache_features") return Promise.resolve()
            if (cmd === "selection_set") return Promise.resolve({ added: [1], removed: [] })
            if (cmd === "selection_apply") return Promise.resolve({ added: [], removed: [] })
            if (cmd === "selection_get_ids") return Promise.resolve([1])
            if (cmd === "selection_count") return Promise.resolve(1)
            return Promise.resolve()
