...
describe("useFeatureProperties", () => {
    ...
    it("preserves distinct features with undefined id on mousemove", () => {
        const map = makeMockMap([
            { id: undefined, source: "s", sourceLayer: "", properties: { x: 1 } },
            { id: undefined, source: "s", sourceLayer: "", properties: { x: 2 } },
        ])
        const ref = makeRef(map)
        renderHook(() => useFeatureProperties(ref, ["layer-1"], 0))
        dispatch.mockClear()
    ...
    it("preserves distinct features with undefined id, same properties but different geometries from same source on mousemove", () => {
        ...
        expect(dispatch).toHaveBeenCalledWith(expect.objectContaining({ payload: { values: [{ x: 1 }, { x: 1 }] } }))
    })
